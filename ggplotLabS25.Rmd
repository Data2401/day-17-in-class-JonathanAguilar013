...
```{r}
ggplot(heights, aes(x = height, group = sex, color = sex, fill = sex)) + geom_density()
...
```{r}
ggplot(heights, aes(x = height, group = sex, color = sex, fill = sex, alpha = 1/2)) + geom_density()
...
```{r}
ggplot(heights, aes(x = height, group = sex, color = sex, fill = sex, alpha = 1/3)) + geom_boxplot()
```
...
```{r}
summarise(heights, mean(height), median(height))
```

Find the mean and median for both groups. 

```{r}
heights %>% group_by(sex) %>% summarise(mean(height),median(height))
```
